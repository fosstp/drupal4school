    ...
    public function buildForm(array $form, FormStateInterface $form_state)
    {
        ...
        if ($config->get('enable')) {
            $form['help'] = array(
                '#type' => 'item',
                '#title' => '提示訊息：',
                '#markup' => '進行帳號與密碼同步到 G Suite 時，會花費較久的時間，請耐心等候同步作業完成。未完成前請勿離開此頁面、重新整理頁面或是關閉瀏覽器！同步作業程序：1. 以臺北市教育人員單一身分驗證服務的帳號搜尋 G Suite，帳號相同者使用現有帳號，如果搜尋不到則自動幫您建立帳號。2. 搜尋 G Suite 群組的說明(description)欄位是否與校務行政系統裡的所屬部門名稱相同，若相同則使用該群組，如果找不到則自動幫您建立群組。（如果您已經有一個匹配的 G Suite 群組，請在說明欄輸入所屬部門名稱，以便讓程式可以正確辨識）3. 移除群組裡所有舊的成員。4. 重新將使用者加入到群組裡。同步程式無法同步密碼。',
            );
