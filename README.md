...
* thumblink 模組：提供一個新的內容類型和一個區塊，用於處理縮圖連結。
* tpedunews 模組：為您啟用 aggregator 模組，將教育局最新消息６個 RSS feed 新增到新聞聯播中，在主導覽選單建立「教育局最新消息」連結。

還在開發的模組有：行事曆（與 G Suite 線上同步功能）、最新消息
